...
#             print("✅ 响应类型为PDF，Cookie有效！")
#             return True
#         else:
#             print(f"❌ 响应不是PDF，Content-Type：{content_type}")
...
#     print(session.headers.get("cookie"))
#     print(response)
#
...
def task():
    ...
    # # 第三步：如果Cookie失效，尝试刷新
    # if not is_valid:
    #     logger.warning("⚠️ Cookie失效，尝试刷新Cookie...")
    #     refresh_success = refresh_cookie(session)
    #     if refresh_success:
    #         # 刷新后再次验证
    #         verify_cookie(session)
    #     else:
    #         logger.critical(" Cookie刷新失败，需要手动检查！")
